...
class Application(Frame):
    ...
    def get_guess(self):
        """Obtain the player's guess and Check it."""
        try:
            guess = int(self.guess_ent.get())
        except(ValueError):
            self.display_message("Invalid entry. Please try again.")
        else:
            self.tries += 1
            Label(self,
                  text="Number Of Attempts: " + str(self.tries)
                  ).grid(row=0, column=3, columnspan=1, sticky=W)
